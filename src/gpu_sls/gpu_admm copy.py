...
def adaptive_rho_update(rp_norm, rd_norm, rho,
    ...
    scale = jnp.sqrt(rp_norm / rd_eff * 2)
    # scale = rp_norm / rd_eff
    # jax.debug.print("DESIRED SCALING: {}", scale)
    # scale = jnp.clip(scale, 0.5, 2.0)
    scale = jnp.clip(scale, 0.9, 10.0)
    rho_propose = jnp.clip(rho * scale, rho_min, rho_max)
    # jax.debug.print("NEW PROPOSED RHO {}", rho_propose)
    # TODO: Maybe include some scaling in this number?
    # updated = jnp.abs(1 - scale) > 0.01
    updated = rho_propose != rho
    # rho_new = lax.cond(updated, lambda _: rho_propose, lambda _: rho, operand=None)
    return rho_propose, updated
...
def constrained_solve(cfg: ADMMConfig, Q, q, R, r, M, A, B, c, C, D, f, w, y, rho):
    ...
    def one_iter(carry):
        ...
        # Convergence check
        converged = jnp.logical_and(rp_norm <= eps_pri, rd_norm <= eps_dual * 10)
