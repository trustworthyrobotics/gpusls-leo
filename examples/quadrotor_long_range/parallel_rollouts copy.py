...
def main():
    ...
    cfg = MPCConfig(
        ...
        N=N,
        W=W,
        u_ref=jnp.array([MASS * GRAVITY, 0.0, 0.0, 0.0]),
        dt=dt,
    )
    ...
    u_min = jnp.array([0.0, -tau_max, -tau_max, -tau_max])
    u_max = jnp.array([T_max, tau_max, tau_max, tau_max])
    ...
    controller = GenericMPC(
        ...
        U_in=jnp.zeros((cfg.N, cfg.nu)).at[:, 0].set(T_hover),
    )
