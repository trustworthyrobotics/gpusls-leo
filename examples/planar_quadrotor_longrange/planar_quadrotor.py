...
def main():
    ...
    for i in range(N_ROLLOUTS):
        ...
        for k in range(T_steps):
            print("STEP:", k)
            # disturbance_history: list of (n,)
            dist_hist = jnp.stack(disturbance_history[:k+1])        # (k+1, n)
            Phi_slice = Phi_u[k, :k+1]                              # (k+1, nu, n)

            disturbance_feedback = jnp.einsum('jn, jmn -> m', dist_hist, Phi_slice)
