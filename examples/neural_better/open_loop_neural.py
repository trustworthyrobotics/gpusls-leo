...
from gpu_sls.external.ReachDev.envs.T_pushing.t_sim import (
    generate_init_target_states,
    T_Sim,
)
...
def main(config: DictConfig):
    ...
    def trans_fn(env_dict):
        ...
        if pred_mode == "pose":
            obj_state = jnp.array(
                np.concatenate(
                    [env_dict["com_pos"] / scale, env_dict["angle"]],
                    axis=0,
                )
            )
            env_state = np.concatenate(
                [
                    np.array(env_dict["com_pos"]),
                    np.array(env_dict["angle"]),
                    env_dict["pusher_pos"],
                ],
                axis=0,
            )
            ...
            env_state = np.concatenate(
                [env_dict["state"][:state_dim], env_dict["pusher_pos"]],
                axis=0,
            )
    ...
    x_min = -x_max
    x_min = x_min.at[1].set(0.6)   # object y >= 0.6

    constraints = make_state_box_constraints(x_min, x_max)
    ...
    # One MPPI solve only, used as warm start
    ...
    # Warm-start solver from MPPI
    controller.X_in = X_init
    controller.U_in = U_init

    # Clip reference to satisfy y >= 0.6
    y_min = 0.6
    reference = X_init.at[:, 1].set(jnp.maximum(X_init[:, 1], y_min))
    ...
    if (
        ...
        print("[WARN] Solver failed; falling back to MPPI controls.")
        X_pred = reference
        U_pred = U_init
        u0 = U_pred[0]
        solver_status = "mppi_fallback"
    else:
        solver_status = "solver_open_loop"

    print("=" * 60)
    print("Single solver solve, full open-loop rollout")
    print("=" * 60)
    print(f"Initial position: {np.array(z_cur)}")
    print(f"Goal position:    {np.array(target_obj)}")
    print(f"MPPI u[0]:        {np.array(U_init[0])}")
    print(f"Used u[0]:        {np.array(u0)}")
    ...
    # Execute the entire solver-predicted horizon open-loop
    for k in range(horizon):
        ...
        print(f"step {k:02d} | action = {action} | state = {np.array(z_next)}")
