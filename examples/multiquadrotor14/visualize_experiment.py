...
#     N_QUADS,
#     SINGLE_N,
#     filename="multi_quad_xy_tubes.png",
# ):
#     """
#     Plot ONLY (px, py) for each quad in a 2-column layout.

#     disturbed: (n_rollouts, T, n)
#     X_pred:    (T+1, n)
#     tube:      (T+1, n)
#     """
...
#     n_rollouts, T, n = disturbed.shape
...
#         # nominal
#         px_nom = X_pred[:T, px_idx]
#         py_nom = X_pred[:T, py_idx]

#         # tube bounds
#         px_lower = lower[:T, px_idx]
#         px_upper = upper[:T, px_idx]

#         py_lower = lower[:T, py_idx]
#         py_upper = upper[:T, py_idx]

#         # --- disturbed rollouts ---
#         for i in range(n_rollouts):
#             ax.plot(t, disturbed[i, :, px_idx], alpha=0.2, linewidth=1)
#             ax.plot(t, disturbed[i, :, py_idx], alpha=0.2, linewidth=1)

#         ax.plot(t, px_lower, linewidth=2, label="lower")
#         ax.plot(t, px_upper, linewidth=2, label="upper")
#         ax.plot(t, py_lower, linewidth=2, label="lower")
#         ax.plot(t, py_upper, linewidth=2, label="upper")

#         # --- plot px ---
#         ax.fill_between(t, px_lower, px_upper, alpha=0.2)
#         ax.plot(t, px_nom, linewidth=2, label="px")

#         # --- plot py ---
#         ax.fill_between(t, py_lower, py_upper, alpha=0.2)
#         ax.plot(t, py_nom, linestyle="--", linewidth=2, label="py")

#         ax.set_title(f"Quad {q}")
...
#     plt.tight_layout()
#     plt.savefig(filename, dpi=200)   # reduced dpi to avoid giant images
#     plt.close(fig)
